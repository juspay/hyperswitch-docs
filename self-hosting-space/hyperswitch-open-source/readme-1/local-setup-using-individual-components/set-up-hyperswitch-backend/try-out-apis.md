---
description: >-
  Explore Hyperswitch APIs using Postman to create payments, refunds, and manage
  merchant accounts
...
3. While on the "Variables" tab, add the admin API key you configured in the application configuration under the "current value" column for the `admin_api_key` variable.
   1. If you're running Docker Compose, you can find the configuration file at [`config/docker_compose.toml`](https://docs.hyperswitch.io/blob/main/config/docker_compose.toml), search for `admin_api_key` to find the admin API key.
   2. If you set up the dependencies locally, you can find the configuration file at [`config/development.toml`](https://docs.hyperswitch.io/blob/main/config/development.toml), search for `admin_api_key` to find the admin API key
