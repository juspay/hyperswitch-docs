...
Log into your AWS account and create a new EC2 instance preferably on a t3.medium machine with an AMI that supports docker like Amazon Linux and a storage of t4g medium.

<figure><img src="../../.gitbook/assets/Screenshot 2023-11-20 at 5.43.04 PM.png" alt=""><figcaption><p>Create and launch an EC2 Instance</p></figcaption></figure>
...
* Create an RDS with the latest `postgres` preferably with `Aurora` \
  (record the master username and password securely for further use in setup)

<figure><img src="../../.gitbook/assets/Screenshot 2023-11-20 at 5.53.56 PM.png" alt=""><figcaption></figcaption></figure>

* To run the migrations install `psql` in the instance or from anywhere else
...
and paste the contents from the following mentioned migration files
...
* Create an `envfile` in the instance and paste the environment variables mentioned below
...
function kms_encrypt() {
    xargs -I {} -0 aws kms encrypt --key-id <your-kms-key-id> --region <your-kms-region> --plaintext $(echo -n '{}' | base64)
}
...
echo -n "<generated master key>" | kms_encrypt # As mentioned below in the local setup guide
...
The following commands are to be used to provide keys
...
If the last command replies with `Decrypted Successfully`, we are ready to use the locker.
