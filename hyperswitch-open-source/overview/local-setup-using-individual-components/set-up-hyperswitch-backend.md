---
icon: server
description: Configure the Hyperswitch server using Docker Compose or Rust environment to process payments locally
---
...
4. Start all the services using Docker Compose:
   ...
   This will compile the payments router, the primary component within Hyperswitch and then start it. Depending on the specifications of the machine, compilation can take around 15 minutes.
...
6. Verify that the server is up and running by hitting the health endpoint:
   ...
   If the command returned a `200 OK` status code, proceed with [trying out the APIs](set-up-hyperswitch-backend.md#try-out-the-apis).
...
1. Install the stable Rust toolchain using `rustup`:
   ...
   ```
    ...
    _Be careful when running shell scripts downloaded from the Internet. This script is only suggested as there seems to be no `rustup` package available in the Ubuntu package repository._
