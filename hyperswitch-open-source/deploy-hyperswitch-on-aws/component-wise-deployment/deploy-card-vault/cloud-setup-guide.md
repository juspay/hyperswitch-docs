---
description: Manually configure and deploy the Juspay Hyperswitch Card Vault on AWS EC2 to establish secure PCI DSS compliant storage for sensitive payment data
---

# Cloud setup guide

{% hint style="info" %}
This guide helps to set up the card vault on AWS manually by setting up the various components
{% endhint %}
...
{% hint style="info" %}
It is recommended to generate the Master and JWE/JWS keys as mentioned below in the local setup guide outside of this EC2 machine for better security
{% endhint %}
...
<pre class="language-bash"><code class="lang-bash"># temporarily turn off saving to history to run the following commands
unset HISTFILE
