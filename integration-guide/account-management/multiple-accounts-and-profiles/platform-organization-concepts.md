...
A **Platform Organization** is a special type of organization in Hyperswitch designed for businesses that onboard and manage multiple merchants programmatically. Think of it as a meta-organization that can:

* Create and manage other Merchant Accounts under its umbrella
* Generate API keys not just for itself, but for the merchants it creates
* Host exactly one Platform Merchant Account that creates and manages multiple sub-merchants, classified as either **Connected** or **Standard**
* Operate in two modes:
  * The Platform Merchant uses its own Platform API Key to directly manage and operate Connected merchants
  * The Platform Merchant maintains merchant-specific API key mappings to perform payments, connector configuration, and profile management for both Connected and Standard merchants, with correct merchant scoping
...
The Platform API Key is a privileged credential owned by the Platform Merchant. Its scope:
...
For how to generate one, see [Setting Up a Platform Organization](setting-up-platform-organization.md).

***

### Merchant Classification: Connected vs Standard

Every merchant account created under a Platform Organization must be classified as either **Connected** or **Standard** at creation time. The classification controls how customers and payment methods behave across the org and what the Platform Merchant can do on the merchant's behalf.
...
* Multiple sub-merchants, each configured as Connected or Standard

#### Platform Merchant

The Platform Merchant is the privileged parent merchant within a Platform Organization. It:

* Owns the shared resource scope for Customers and Payment Methods
* Performs management operations across the platform (merchant creation, governance, administration)
* Can act on behalf of Connected merchants for operational capabilities
* Does not replace a regular merchant. It's a control-plane entity with optional operational capabilities, and it cannot process payments for itself.
...
* Centralised orchestration with clear ownership and attribution (platform acting on behalf when needed)
