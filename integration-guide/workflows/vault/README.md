...
* **Network Tokenization** - When it is enabled for the profile and the token service is configured, saving an eligible card can provision a network token. See [Network tokenisation](../../../other-features/network-tokenisation.md) for prerequisites, supported defaults, and fallback behavior.
