...
Call `paymentSession.getCustomerSavedPaymentMethods`, keep the `PaymentSessionHandler` it passes to your callback, and work from that handler. It carries the saved payment method data and the confirm functions, as the example below shows. `paymentSession` itself does not expose them.
