...
<!-- truth manifest; hyperswitch 5fb7e5598eadd8ed5fa42822427107f271a1e112; spec crates/openapi/src/routes/payments.rs@5fb7e5598eadd8ed5fa42822427107f271a1e112
     ...
     absent: server-side confirmation of a standalone Web wallet button = checked hyperswitch-web@e8e869329c44aa769efe3df89f98e4fd51de6d04. Closed by mechanism rather than by not finding one. (a) On success the SDK-button path always redirects: the isSdkButton branch at src/hyper-loader/Hyper.res:442-443 calls replaceRootHref before the redirect==always test is reached, so resolve1(data) at :449 is unreachable for a button and no result returns to the page. (b) confirmPayloadForSDKButton forces redirect always and carries only return_url, src/Utilities/PaymentBody.res:315-324. (c) confirmPaymentViaSDKButton is not on the public hyper instance, src/hyper-loader/Hyper.res:492 defines it and :500 is its only caller, from the handleSdkConfirm postMessage listener; the 14 public members are at :827-842. Ruled out as false leads: onpaymentauthorized is Apple's own ApplePaySession callback, src/Utilities/ApplePayHelpers.res:208,469, not a merchant hook; the window.fetch patching in src/hyper-loader/ApplePayInterceptor.res:118-150 is TrustPay connector plumbing. Scope: the JS surface at this SHA only, which does not speak to any server-side API accepting a wallet token by another route
     checked: 2026-09-17 -->
