...
- **HTTP Basic auth with username and password:** Klarna's `BodyKey` auth maps key1 to the API username and api_key to the API password. The authorization header is `Authorization: Basic base64(username:password)`. Both credentials are required — neither alone is sufficient.
...
Symptom: All requests fail with a Klarna authentication error. Fix: Verify both the API Username and API Password in Hyperswitch match exactly what is configured in your Klarna merchant portal under **Settings → API credentials**. Do not manually Base64-encode them before entering in the control center.

**Klarna credit decision rejection**
Symptom: Payment fails with a Klarna-specific rejection code before authorization. Fix: Klarna makes its own credit decision for BNPL payments based on the customer's profile. A rejection here is not a card or gateway failure — it cannot be retried with the same payment method.

**Payment method not available**
Symptom: A specific Klarna option (Pay Later, Installments) is not offered for a country or currency. Fix: Verify Klarna's availability for the specific country-currency combination is enabled for your merchant account. Klarna's product availability varies by market.
