...
Hyperswitch attempts to verify the `hashed_order` value with HMAC-SHA256, but the current implementation does not match OpenNode's contract: it computes the HMAC over the raw request body keyed by the connector's `merchant_secret`, while [OpenNode's webhook documentation](https://developers.opennode.com/docs/charges-webhooks) specifies the charge `id` as the message and the API key as the key. Additionally, the webhook payload parser requires fields (`payment_method`, `net_fiat_value`, `fiat_value`) that OpenNode does not document sending, so documented payloads may fail deserialization before event dispatch. We are working on fixing the connector implementation; see [the tracking issue](https://github.com/juspay/hyperswitch/issues/14205). Until then, legitimate callbacks fail signature verification and are processed as source-unverified, so treat payment status updates as relying on payment sync for now. See [`get_webhook_source_verification_signature()`](https://github.com/juspay/hyperswitch/blob/e8e30d1018b1ab5aecada04cf1b3ab63a39a68d0/crates/hyperswitch_connectors/src/connectors/opennode.rs#L396-L406) and [`get_webhook_source_verification_message()`](https://github.com/juspay/hyperswitch/blob/e8e30d1018b1ab5aecada04cf1b3ab63a39a68d0/crates/hyperswitch_connectors/src/connectors/opennode.rs#L408-L417).
