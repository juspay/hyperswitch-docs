...
![logo\_opennode](https://hyperswitch.io/icons/homePageIcons/logos/opennodeLogo.svg)
...
Symptom: All requests fail with an OpenNode authentication error. Fix: Verify the API Key in Hyperswitch matches the one in your OpenNode dashboard. Note that OpenNode uses a raw Authorization header (no "Bearer " prefix) — the Hyperswitch implementation handles this format automatically.

**Webhook verification failure**
Symptom: OpenNode events arrive at Hyperswitch but payment statuses do not update. Fix: Verify the Hyperswitch webhook endpoint URL is configured correctly in your OpenNode dashboard and that the webhook secret matches.
