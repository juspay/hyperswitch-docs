# create_recipient Method

<!--
---
title: create_recipient (Python SDK)
description: Create a recipient entity for payouts. Using the Python SDK.
...
The `create_recipient` method registers a new recipient entity (individual or business) with the payment processor. This is often required before funds can be transferred to them.
...
|----------|--------------------------|
| Onboard a new seller | Call `create_recipient` with their details and `recipient_type`. |
...
response = await payout_client.create_recipient(request)
```
