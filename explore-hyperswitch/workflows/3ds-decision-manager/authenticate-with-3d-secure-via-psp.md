...
3. If 3DS is:
   1. Not required: For example, because of an [exemption](3ds-intelligence-engine/), Juspay Hyperswitch attempts the payment as a non-3DS transaction. The Payment transitions to a status of `succeeded`. If requested by the issuer with a soft decline, we automatically reattempt and continue as if required.
   ...
   3. Required: Juspay Hyperswitch starts the 3DS authentication flow by contacting the card issuer’s 3DS Access Control Server (ACS) and starting the 3DS flow
