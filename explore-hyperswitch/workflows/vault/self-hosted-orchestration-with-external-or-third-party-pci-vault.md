...
<table><thead><tr><th width="204.40625">Concern</th><th>How It's Addressed</th></tr></thead><tbody><tr><td><strong>PCI Compliance</strong></td><td>Fully offloaded to the vault provider (VGS, Tokenex, etc.)</td></tr><tr><td><strong>Hosting Independence</strong></td><td>Merchant retains complete control of the Juspay Hyperswitch deployment</td></tr><tr><td><strong>Sensitive Data Exposure</strong></td><td>Raw card data never enters the merchant's environment</td></tr><tr><td><strong>Token Portability</strong></td><td><code>payment_method_id</code> unifies <code>vault_token</code> + <code>psp_token</code> + <code>customer_id</code> for cross-platform use</td></tr><tr><td><strong>Operational Simplicity</strong></td><td>No need to manage HSMs, key rotation, or cardholder data environments</td></tr></tbody></table>
...
* **Any vault with a proxy/detokenize API** — Juspay Hyperswitch's modular vault architecture supports any vault that exposes a tokenize + proxy (or detokenize) HTTP API.

Juspay Hyperswitch's modular vault architecture supports configuring multiple vault connectors simultaneously. You can route different merchants or payment methods to different vaults based on your business logic.

### Integration Steps

#### Option 1: Merchant Managed Client

The merchant client/checkout integrates the external vault SDK **directly** which takes away the PCI scope for the merchant, collect the card details and tokenizes it. The merchant server then passes the resulting token and metadata to Juspay Hyperswitch Backend for payment processing.
...
* The merchant calls the [Payments Create API](https://api-reference.hyperswitch.io/v1/payments/payments--create) to send the `vault_token` and card metadata to the self-hosted Juspay Hyperswitch backend.
* Juspay Hyperswitch constructs the PSP payload using the `vault_token`.
...
* The PSP responds with `approved` or `declined` along with a `psp_token`. The vault proxy relays this response back to Juspay Hyperswitch.
* Juspay Hyperswitch generates a `payment_method_id` linking `customer_id`, `vault_token`, and `psp_token`.
...
* Juspay Hyperswitch resolves the `payment_method_id` to identify the associated `psp_token` and processes the payment by sending the transaction to the corresponding PSP as a MIT
...
* The Hyperswitch SDK sends a [Payment Confirm API](https://api-reference.hyperswitch.io/v1/payments/payments--confirm) request containing the `vault_token` and card metadata to the self-hosted Juspay Hyperswitch backend.
* Juspay Hyperswitch constructs the PSP payload using the `vault_token`.
* The PSP payload (containing the `vault_token`) is sent to the **Proxy endpoint** of the external vault.
...
A **universal, portable token** generated by Juspay Hyperswitch that serves as the single reference for a stored payment method. It connects:

| Entity        | Description                                                         |
| ------------- | ------------------------------------------------------------------- |
| `customer_id` | The Juspay Hyperswitch customer identifier                                 |
...
To enable an external vault with your self-hosted Juspay Hyperswitch instance:

1. **Navigate to the Juspay Hyperswitch Dashboard:** `Orchestrator → Connector → Vault Processor`
...
4. **Test the integration** — Use the Juspay Hyperswitch Payments API with the external vault token to verify the payment flows with the various PSP integrations
...
* **TLS Everywhere:** All communication between Juspay Hyperswitch, the vault, and PSPs must use TLS 1.2+.
* **Credential Rotation:** Regularly rotate vault API keys and proxy credentials.
* **Audit Logging:** Enable Juspay Hyperswitch audit logs for all vault operations (tokenize, detokenize, proxy calls) to maintain compliance evidence.
* **Network Segmentation:** Even though PCI is outsourced, restrict network access between your Juspay Hyperswitch deployment and the vault to only the required endpoints and ports.
...
**Q: Can I switch vault providers without affecting existing customers?** A: Yes. Since Juspay Hyperswitch uses `payment_method_id` as the universal reference, you can migrate vault tokens between providers. Juspay Hyperswitch will update the internal mapping from `payment_method_id` → new `vault_token`.
...
**Q: Can I use multiple vault providers simultaneously?** A: Juspay Hyperswitch's modular vault architecture supports configuring multiple vault connectors. You can route different merchants or payment methods to different vaults based on your business logic.
