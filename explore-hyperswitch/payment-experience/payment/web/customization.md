---
description: Customize the web checkout experience to match your brand and optimize customer conversions
icon: bars-progress
---
...
### One Click Payment Methods Customization
...
| applePay: showType<br>googlePay: showType<br>payPal: showType<br>klarna: showType               | Determines the visibility of Apple Pay, Google Pay, Paypal and Klarna                                                                                                                             | `showType` can take two values:<ul><li>`"auto"`: Display when supported</li><li>`"never"`: Always hidden</li></ul>                                                             |
| style: {<br>theme: theme,<br>type: styleType,<br>height: int,<br>buttonRadius: int,<br>}        | Configures the wallet's appearance with the following options:<ul><li>`theme`: Sets the theme</li><li>`type`: Defines the style type (e.g. buy)</li><li>`height`: Specifies the height of the wallet</li><li>`buttonRadius`: Adjusts the button corner radius</li></ul> | `theme`: It can take values as `dark`, `light`, or `outline`<br><br>`type`: Specifies the wallet button style with options including `checkout`, `pay`, `buy`, `installment`, `default`, `book`, `donate`, `order`, `addmoney`, `topup`, `rent`, `subscribe`, `reload`, `support`, `tip`, and `contribute` |
