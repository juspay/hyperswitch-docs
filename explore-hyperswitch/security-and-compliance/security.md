...
* **Enhanced Data Security**: Juspay Hyperswitch employs advanced encryption layers to secure data during transmission and storage.
...
Card information is initially received from the Hyperswitch SDK. This data is encrypted using SSL/TLS protocols, ensuring end-to-end encryption during transmission between the SDK and the Juspay Hyperswitch backend. This guarantees that sensitive card data is secure from the point of collection.

#### **2. Storing Card Data (Vaulting)**

When a payment is made using a saved card, the card details are securely stored in the Juspay Hyperswitch Card Vault. This process involves multiple steps to ensure the confidentiality and integrity of the data:

**Card Data Preparation:**

* **Signing for Integrity**: The Juspay Hyperswitch App Server signs the card details using its private key, ensuring the data has not been altered.
...
1. **Validation**: The Juspay Hyperswitch Card Vault validates the signed data using the public key of the Juspay Hyperswitch App Server. This step ensures the integrity of the transmitted data.
...
For more technical details about how the Juspay Hyperswitch Card Vault manages encryption and decryption, you can visit the [Hyperswitch Card Vault GitHub repository](https://github.com/juspay/hyperswitch-card-vault).
...
Juspay Hyperswitch employs AWS Key Management System (KMS) to securely manage sensitive keys required for the application’s operation.
...
Juspay Hyperswitch utilizes a **Key Manager Service** to ensure the secure generation and storage of a **unique Data Encryption Key (DEK)** for each merchant.
...
* **Leveraging Rust's Advantages**: Juspay Hyperswitch uses Rust, a strongly typed programming language, to create a robust mechanism (`Secret<T>`) for handling sensitive data.
