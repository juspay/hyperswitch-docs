...
* Multi-Connector Retry: If multiple connectors are available for a payout method, Juspay Hyperswitch attempts the retry using the next available connector in your priority list.
...
* Error Configuration: Retries are only initiated for error codes that have been loaded into the Juspay Hyperswitch engine. Merchants can submit custom error configurations to the Juspay Hyperswitch team.
