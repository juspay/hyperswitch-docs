---
description: >-
  Secure tokenization using Hyperswitch's PCI compliant Vault SDK for React
  applications with REST API integration.
...
app.post(`/create-payment-methods-session`, async (req, res) => {
  try {
    const response = await fetch(
      `https://sandbox.hyperswitch.io/v1/payment-method-sessions`,
      {
        method: "POST",
        headers: {
