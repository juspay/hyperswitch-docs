...
{% hint style="info" %}
With this section, understand how the Juspay Hyperswitch Smart Router works to improve your conversion rates and reduce processing costs by intelligently routing payments across various processors
{% endhint %}
...
To get started with Smart Router, ensure you have one or more payment processors integrated. You can integrate the payment processor of your choice on the Control Center by following the [Connector Integration](../hyperswitch-cloud/connectors/) guide.

## What is Smart Payment Routing?

Selling globally invariably brings in a requirement to adopt multiple payment processors to cater to a wide range of payment method needs of the customers. It gives you the flexibility to switch between processors to manage downtime and optimize your payment processing costs as your business can choose the most optimal payment processors for every payment based on the cost, region and customer.

Hence, Juspay Hyperswitch's smart router is designed as a no-code tool to provide complete control and transparency in creating and modifying payment routing rules. Juspay Hyperswitch supports the below formats of Smart Routing.

**Volume Based Configuration:** Define volume distribution among multiple payment processors using percentages.

**Rule Based Configuration:** More granular control which allows you to define custom routing logic based on different parameters of payment.
...
**Cost Based Configuration** (submit a feature request [here](https://github.com/juspay/hyperswitch/discussions/new?category=ideas-feature-requests))**:** Automatically routes transactions to the payment processor charging the least MDR (merchant discount rate) for the opted payment method.
...
[Juspay Hyperswitch dashboard](https://app.hyperswitch.io/routing) provides a simple, intuitive UI to configure multiple Routing rules on your dashboard under the **Routing** tab. There are three routing rule formats that Juspay Hyperswitch currently supports.
...
To test the Smart Router, after activating one rule, you can make a Test Payment using the [Juspay Hyperswitch Dashboard](https://app.hyperswitch.io/sdk)
...
- Now you go ahead and open up the Juspay Hyperswitch SDK to make a test payment. In the payment method selection area, you can see two buttons, one for `Cards` and one for `ApplePay`.
...
- Now you select `ApplePay`, go through the required steps, confirm the payment, and it succeeds. You go to your payments dashboard and see that the payment went through `Adyen`.
  ...
  - The answer to this carries over from the previous point. Since we displayed `ApplePay` on the checkout screen even though it wasn't enabled for your preferred connector `Stripe`, we need to adhere to your connector payment method configuration and ensure the payment goes through the right connector, here, `Adyen` since `ApplePay` is only enabled for Adyen. To briefly explain how Juspay Hyperswitch reaches this conclusion:
    - Juspay Hyperswitch runs your configured routing algorithm. Since this is `100% Stripe`, Juspay Hyperswitch receives `Stripe` as the output.
